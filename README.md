...
- candidate-workspace implementation is completed through `P8-007` / `P8-007B` as a frontend-only workspace, shortlist/notes/statuses, deterministic explanations, bounded selected-candidate wording, and local export workflow; next Phase 8 work should continue from reviewed backlog tasks rather than changing persistence or backend search behavior implicitly;

...
- Post-deploy hardening through Phase 9.14 is completed: global LocationGuard is active, non-developer IT briefs can derive safe executable search signals from explicit requirements such as banking domain, English, and Excel, IT-adjacent role scope is checked through bounded Role Understanding, harmless help/small-talk openings get friendly onboarding before the unclear-search fallback, and natural Search Brief replacement requests such as `Change Angular skill to PHP` are resolved through bounded interpretation plus backend validation while preserving human approval before execution.

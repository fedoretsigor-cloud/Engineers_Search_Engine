...
- `P8-007` is drafted/reviewed as a local frontend-only export workflow: Excel-on-Windows-friendly CSV plus deterministic Markdown, explicit `visible`/`shortlisted`/`all` scopes, allowlisted fields only, no raw candidate payload dump, no persistence, no backend/API/search/runtime/Tavily/LinkedIn calls, and no outreach/account actions.
...
- candidate-workspace implementation is completed through `P8-005`; next Phase 8 review should continue from contract-first candidate explanation wording, local export implementation, or other reviewed backlog tasks rather than changing persistence or backend search behavior implicitly;

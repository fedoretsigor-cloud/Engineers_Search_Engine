...
Candidate explanation path: deterministic candidate-level explanations are implemented in `P8-005`, grounded only in returned workspace facts. The wording path is split into completed docs-only `P8-006 Define bounded candidate explanation wording contract` and implemented `P8-006.1 Implement explicit selected-candidate wording overlay`. LLM wording can only rewrite deterministic explanation wording after validation and fallback. LLM wording must not decide facts, stable `reason_key`, reason codes, source/version, fit, score, ranking, filters, search execution, review state, notes, shortlist, export, or persistence, and it must not run just because a candidate is opened or selected.
...
`P8-006` is contract-first and docs-only. It defines the payload, output shape, validation, fallback, routing, provenance, and implementation handoff before any code starts. `P8-006.1` is implemented as the explicit selected-candidate wording overlay.

The approved endpoint for `P8-006.1` is:

```text
POST /api/candidate-workspace/explanation-wording
```

The endpoint is intentionally scoped to Candidate Workspace wording. It is not an Agent Runtime endpoint and must not prepare, approve, execute, search, export, persist, message, open profiles, or perform account actions.
...
The implementation should include a drift check that compares the implemented candidate-explanation wording semantics snapshot against `EXPLANATION_REASON_CODES`. If a future code change adds, removes, or renames a reason code, the wording overlay tests should fail until `Tasks.md`, this contract, the mapper, prompt, validator, and semantic guardrail tests are reviewed and updated together.
...
The guardrail coverage should also assert snapshot parity with the exported `EXPLANATION_REASON_CODES` allowlist so a new or renamed deterministic reason code cannot reach wording without a reviewed contract update.
...
The first `P8-006.1` implementation should not add backend session storage or backend in-memory candidate workspace storage. Overlay reuse should be frontend current-run memory only. A backend-owned `backend_wording_cache_key` may be returned as provenance/correlation metadata if useful, but it must not imply persistent or cross-request backend candidate memory.

The frontend should build the wording request from the structured deterministic explanation helper output, not from rendered DOM text. Candidate explanation wording v1 should send target language `en`; other languages remain unsupported/no-call until a later reviewed localization task. Deterministic and overlay text must be rendered with the same escaping/plain-text safety and never inserted as raw HTML.
...
Review status: `P8-007` is implemented and completed. `P8-007A` completed the DOM-free export model, CSV/Markdown serializers, filename/MIME helpers, and no-network helper smoke coverage. `P8-007B` completed the explicit UI/download task with grouped export controls, current-run export state, click-time visible recomputation, local Blob/object URL download glue, bounded inline status, CSS, and no-network frontend wiring smoke coverage.
...
`P8-007A` implementation status: completed in `app/static/candidate_workspace.js` with helper smoke coverage in `scripts/smoke_p8_candidate_workspace_helpers.js`. The completed slice adds only DOM-free helper/model/serializer behavior and intentionally does not add backend/API/search/runtime calls, persistence, browser storage, or external-service behavior.

`P8-007B` implementation status: completed in `app/static/app.js` and `app/static/styles.css`, with no-network frontend wiring coverage in `scripts/smoke_p8_export_ui_wiring.py` and `scripts/check_all.ps1`. The completed slice adds only explicit local export UI/download glue on top of the `P8-007A` helpers: grouped controls, current-run `workspaceExportState`, export-specific data attributes, click-time visible recomputation, local `Blob`/object URL/temporary-anchor download, bounded statuses, stale-status clearing, and cleanup. It does not add backend/API/search/runtime/Tavily/OpenAI/LinkedIn calls, persistence, browser storage, CRM/share behavior, outreach, account actions, or autonomous execution.
...
Implementation order should stay narrow and testable: DOM-free model/normalization helpers first, CSV/Markdown serializers second, UI state/rendering third, browser download glue fourth, and browser sanity last. Required execution split is now completed: `P8-007A` implemented only DOM-free export model, normalization/sanitization helpers, CSV/Markdown serializers, filename/MIME helpers, and no-network helper smoke coverage; `P8-007B` implemented only export UI controls, current-run `workspaceExportState`, click-time visible recomputation, local Blob download glue, bounded inline status, cleanup, CSS, and frontend wiring checks. Do not expand future export changes beyond these boundaries without a separate reviewed task.

...
- Phase 9.9 - `AI Agent Semantic Understanding Hardening`: draft / not approved for coding. This is the next proposed AI Agent chat-understanding correction after Phase 9.8. It should replace fragile clean-state initial message parsing with a bounded LLM `SearchBriefExtractor v2` plus strict backend validation, so multi-signal requests such as `I need Analyst in Canada with banking domain experience and SQL skills` separate role, location, technical skills, and domain experience correctly. Planned tasks: `P9.9-001` extractor contract, `P9.9-002` strict backend validator, `P9.9-003` clean-state chat integration, `P9.9-004` legacy semantic-branch retirement, `P9.9-005` automated semantic recruiter UAT that Codex must run during verification, `P9.9-006` bounded Search Brief refinement interpreter v2, and `P9.9-007` closeout decision. It must not change planner/scoring, providers, runtime approval, LinkedIn/account boundaries, or persistence.
...
- Phase 9.9: `AI Agent Semantic Understanding Hardening` draft; review/coding only when explicitly approved. Includes bounded clean-state Search Brief extraction, strict validator, clean-state integration, legacy semantic-branch retirement, automated semantic recruiter UAT, bounded refinement interpreter v2, and closeout.

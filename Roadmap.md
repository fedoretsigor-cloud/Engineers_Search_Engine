...
- Phase 7.5 - `Recruiter Simulation QA & Flow Hardening`: перед Phase 8 провести глубокую симуляцию живого рекрутера в локальном браузере на текущем Java/Ukraine Agent flow. OpenAI live calls разрешены для существующих chat/planning/wording paths, Tavily execution запрещен, покрытие обязательно RU + EN. QA останавливается на approval/no-Tavily boundary: recruiter chat -> Search Brief -> Agent Plan -> Build Plan -> visible QueryPlan -> approval/no-Tavily boundary. Цель - собрать QA findings, согласовать исправления текущего flow и только потом принять readiness decision для Phase 8: `ready`, `ready after approved fixes` или `not ready`.
...
- Phase 7.5 добавляет no-Tavily recruiter simulation QA gate: проверяем текущий Java/Ukraine flow как живой рекрутер в RU/EN, с OpenAI для существующих путей, но без Tavily execution.
...
- Phase 7.5: `Recruiter Simulation QA & Flow Hardening`; next task for review is `P7.5-002 Define RU/EN recruiter simulation scenarios`.

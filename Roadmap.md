...
- Phase 9.6 - `Post-Deploy Recruiter UX Polish`: completed narrow post-deploy UI/conversation polish through `P9.6-001` through `P9.6-006`. Candidate Results now shows manual safe LinkedIn profile links under candidate identity, removes low-value `Location` and `Stack` columns from the primary table, tightens the initial chat helper, increases the message input height, reduces chat action button height, and replaces hardcoded stack-signal-only acceptance with bounded LLM IT/software relevance validation where deterministic recognition is insufficient. This preserves manual user-click-only LinkedIn behavior and keeps backend/runtime search approval authoritative.
...
- Phase 9.6: `Post-Deploy Recruiter UX Polish` through `P9.6-006`.

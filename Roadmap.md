...
- Phase 8 - `Candidate Workspace/Table + Shortlist`: сделать таблицу кандидатов главным рабочим artifact после чата: score, evidence, role/tech/location fit, review flags, query/wave source, filters, сортировка, объяснения, shortlist и экспорт. `P8-001` зафиксировал Candidate Workspace v0 contract; `P8-002`, `P8-003`, `P8-004`, and `P8-005` implemented консервативные frontend-only slices: mapper/workspace run state/table/read-only details, workspace view sorting/filtering поверх `workspaceCandidates`, local review state/derived shortlist/escaped notes/statuses, затем deterministic candidate-level explanations на базе returned workspace facts без влияния recruiter review state. Следующий explanation path: `P8-006` contract-first bounded candidate explanation wording, затем отдельный `P8-006.1` explicit-action selected-candidate wording overlay. LLM может только переформулировать уже построенное deterministic explanation без изменения фактов, stable `reason_key`, reason codes, source/version, scoring, filters, review state или execution behavior; opening/selecting candidate must not call OpenAI. Future implementation must separate versioned frontend-to-backend request payload from versioned backend-to-OpenAI model payload, use opaque non-URL `wording_target_key` in the request instead of URL-derived current `candidate_id`, reject frontend-supplied prompt rules/hard boundaries/allowed numbers/OpenAI execution controls, treat candidate/user-derived summary/labels/facts/stack-location-query values as data rather than instructions, keep backend-owned policy/schema instructions structurally separate from bounded data in the model payload, recompute allowed numbers from user-visible wording fields only, recompute request-level explanation fingerprint from sanitized request-bounded fields using UTF-8 canonical JSON/sha256 parity for request integrity/UI correlation only, validate wording-safe allowlisted bounded facts through an explicit facts mapper, keep request correlation/cache/internal fields out of the OpenAI payload, reject model-returned `warnings` in v1, use Phase 7-aligned backend-owned provenance/fallback/no-call metadata, prevent duplicate in-flight requests through a separate `frontend_pending_key`, and reuse validated overlays through a backend-owned `backend_wording_cache_key`. Future wording must be a separate non-mutating current-run overlay, use explicit request/model payload contract versions plus reason semantics/canonicalizer provenance and exact candidate provenance values, enforce strict plain-text output caps with conservative v1 English validation that tolerates technology/location/query tokens, no-call unsupported languages before OpenAI, validate backend-only model payload version internally before OpenAI, and avoid persistent storage/logging or backend-error/frontend-status exposure of raw wording payloads, backend-to-OpenAI model payloads, or raw model responses. This validation is not candidate-fact proof or latest-run proof; backend-owned candidate facts need a later backend producer or persistence task. `P8-007` подготовлен как локальный frontend-only export workflow: Excel-on-Windows-friendly CSV, deterministic Markdown, default `visible` scope, явные `visible`/`shortlisted`/`all` scopes, stable allowlisted fields, no raw candidate payload dump, no persistence/backend/API/search/runtime/Tavily/LinkedIn/outreach/account behavior. Shortlist/notes/statuses в Phase 8 являются browser in-memory session/local UI state до Phase 9, `workspace_run_id` имеет per-run компонент, а profile links остаются manual user-click только после safe LinkedIn URL validation. Узкие chat-flow улучшения вроде chat-confirmed `Build Plan` можно держать в Phase 8 backlog, если они не меняют execution boundary и сохраняют отдельный `Approve & Search` перед Tavily.
...
Критическое решение по дальнейшему пути: сначала доводим до качества один узкий flow `Backend Developer + Java + Ukraine`, а не расширяем страны/технологии. Phase 5 закрыла агентный UX на этом flow: onboarding, clarification, brief refinement, approved search, result-to-next-iteration loop и единый AI Agent visual style. Phase 5.5 модульно подготовила backend. Phase 6 добавила human-approved runtime baseline. Phase 7 закрыла controlled agent conversation wording layer без изменения state, tools, approval, Search Brief, QueryPlan, candidates, counts или execution actions. Phase 7.5 проверила текущий flow глазами рекрутера в RU/EN, нашла blockers, провела approved fixes и закрылась решением `ready after approved fixes completed`. Теперь Phase 8 превращает результаты поиска в рабочий recruiter artifact: candidate workspace/table, shortlist, notes/statuses и будущий export workflow. `P8-001 Define candidate workspace contract`, `P8-002 Build recruiter-facing candidate table`, `P8-003 Add sorting and filtering by quality signals`, `P8-004 Add shortlist, notes, and statuses`, and `P8-005 Add candidate-level agent explanations` are completed. Candidate explanations идут в три шага: deterministic facts first (`P8-005`) completed, bounded wording contract (`P8-006`) before code, explicit-action selected-candidate wording overlay (`P8-006.1`) later. Export path starts from reviewed `P8-007`: local explicit download only, Excel CSV plus Markdown first version, allowlisted current-session workspace data only, no persistence or external actions.
...
- Phase 8: `Candidate Workspace/Table + Shortlist`; current active phase after `P7.5-010`. `P8-001 Define candidate workspace contract`, `P8-002 Build recruiter-facing candidate table`, `P8-003 Add sorting and filtering by quality signals`, `P8-004 Add shortlist, notes, and statuses`, and `P8-005 Add candidate-level agent explanations` are completed. The current implementation is frontend-only workspace/table, sorting/filtering, browser in-memory review state, and deterministic candidate explanations.

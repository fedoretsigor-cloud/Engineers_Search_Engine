...
- Phase 8.5: `Agentic Candidate Review & Iteration`; current active phase after `P8-032` parent closeout. `P8.5-001 Define agentic candidate review contract`, `P8.5-002 Add top-candidate recommendation from returned workspace facts`, `P8.5-003 Add selected-candidate comparison`, `P8.5-004 Add fit/gap explanation across selected candidates`, and `P8.5-005 Add guided next-refinement suggestions from workspace results` are completed. The current implementation remains frontend-only workspace/table, sorting/filtering, browser in-memory review state, deterministic candidate explanations, bounded explicit selected-candidate wording overlay, local export UI/download workflow, bounded recruiter-chat quality hardening, recruiter-facing presentation/conversation/post-search cleanup, deterministic top-candidate recommendation over current visible workspace facts, deterministic selected-candidate comparison over visible shortlisted candidates, deterministic selected-candidate fit/gap explanation, and deterministic non-executable review/refinement guidance from current visible workspace facts.

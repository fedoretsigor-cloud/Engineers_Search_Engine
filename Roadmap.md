...
- Phase 8 - `Candidate Workspace/Table + Shortlist`: сделать таблицу кандидатов главным рабочим artifact после чата: score, evidence, role/tech/location fit, review flags, query/wave source, filters, сортировка, объяснения, shortlist и экспорт. `P8-001` зафиксировал Candidate Workspace v0 contract; `P8-002`, `P8-003`, and `P8-004` implemented первый консервативный frontend-only batch: mapper/workspace run state/table/read-only details, workspace view sorting/filtering поверх `workspaceCandidates`, затем local review state/derived shortlist/escaped notes/statuses. Shortlist/notes/statuses в Phase 8 являются browser in-memory session/local UI state до Phase 9, `workspace_run_id` имеет per-run компонент, а profile links остаются manual user-click только после safe LinkedIn URL validation. Узкие chat-flow улучшения вроде chat-confirmed `Build Plan` можно держать в Phase 8 backlog, если они не меняют execution boundary и сохраняют отдельный `Approve & Search` перед Tavily.
...
- Phase 8: `Candidate Workspace/Table + Shortlist`; current active phase after `P7.5-010`. `P8-001 Define candidate workspace contract`, `P8-002 Build recruiter-facing candidate table`, `P8-003 Add sorting and filtering by quality signals`, and `P8-004 Add shortlist, notes, and statuses` are completed. The current implementation is frontend-only workspace/table, sorting/filtering, and browser in-memory review state.

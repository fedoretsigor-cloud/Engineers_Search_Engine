...
- Phase 8: `Candidate Workspace/Table + Shortlist` - current active phase after Phase 7.5 readiness decision
...
Future phase note after Phase 8 closeout: Phase 8.5 is agentic candidate review/iteration from already returned workspace facts; Phase 9 is persistence/saved searches; Phase 10 is manual user-provided profile evidence paste-in; Phase 11 is resume upload and fit analysis; Phase 12 is multi-provider search through Serper/SerpApi providers. These future tracks must preserve the same human-approved runtime boundary and must not add LinkedIn login, scraping, browser automation, messaging, autonomous execution, or account actions.

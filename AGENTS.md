...
- Next agreed direction: keep the product focused on one narrow Java/Ukraine flow. `P7-001 Define agent message taxonomy and lifecycle mapping` is completed with `docs/phase-7-agent-message-taxonomy.md`, `P7-002 Define message facts and source-of-truth contract` is completed with `docs/phase-7-message-facts-contract.md`, `P7-003 Define agent wording style and language policy` is completed with `docs/phase-7-agent-wording-style-policy.md`, `P7-004 Build deterministic source messages for approved message types` is implemented with `app/agent_messages.py` and `scripts/smoke_p7_agent_messages.py`, `P7-005 Define LLM routing and gating policy for conversation wording` is completed with `docs/phase-7-llm-routing-gating-policy.md`, `P7-006 Add bounded LLM wording payloads and prompt contract` is completed with `docs/phase-7-bounded-llm-payload-prompt-contract.md`, `P7-007 Add wording validation, fallback, and provenance metadata` is implemented with nested `wording_provenance`, strict fallback/no-call semantics, and `scripts/smoke_p7_wording_validation.py`, `P7-008 Add frontend rendering for typed agent messages` is implemented as frontend-only typed chat rendering with inert `next_iteration_options`, and `P7-009 Add golden conversation scenario regression tests` is implemented as no-network scenario-level regression coverage. Next task for review is `P7-010 Close Phase 7 with wording quality and guardrail evaluation`. Ordinary LLM-assisted agent conversation wording must stay inside Phase 7 guardrails. Phase 7 order is contract-first: message taxonomy/lifecycle, facts contract, style policy, deterministic source messages, LLM routing/gating, bounded prompt payloads, validation/fallback/provenance, frontend rendering, golden scenario tests, then closeout. Do not create a separate observability/versioning task yet: carry lightweight wording provenance/version metadata in `P7-007` and assert it in `P7-009`; keep it internal for debugging/regression, not product analytics, telemetry, memory, or user tracking.

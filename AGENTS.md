...
- Phase 8.75: `Recruiter UAT & Acceptance Gate` - completed green before Phase 9; `P8.75-001 Run recruiter UAT acceptance gate before persistence` and `P8.75.1-001 Run UI conversation UX UAT before persistence` are completed
...
`P8.75-001` is completed as the recruiter UAT acceptance gate before Phase 9 persistence. It adds `docs/phase-8-75-uat-acceptance-gate.md`, deterministic no-live UAT in `scripts/uat_phase_8_75_no_live.py`, Candidate Workspace helper UAT in `scripts/uat_phase_8_75_workspace_cases.js`, limited live backend-runtime UAT in `scripts/uat_phase_8_75_live.py`, and the aggregate report `docs/phase-8-75-uat-report.md`. No-live UAT is wired into `scripts/check_all.ps1`; live UAT is intentionally local-only and must use only Agent Runtime prepare -> explicit approval -> execute_approved. Do not commit raw Tavily payloads, profile URLs, candidate URLs, screenshots, or secrets.

`P8.75.1-001` is completed as the real frontend simulated-user conversation UX UAT before Phase 9 persistence. It adds `docs/phase-8-75-1-conversation-ux-uat.md`, `scripts/uat_phase_8_75_1_ui_conversation.py`, and `docs/phase-8-75-1-conversation-ux-report.md`; final UI UAT passed `116/116` scenarios with deterministic OpenAI/Tavily doubles. This runner is local-only because it needs Playwright browser binaries. Do not commit screenshots, raw candidate/profile URLs, raw Tavily/OpenAI payloads, or secrets.

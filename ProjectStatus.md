...
Completed Phase 8 tasks: `P8-001 Define candidate workspace contract`, `P8-002 Build recruiter-facing candidate table`, `P8-003 Add sorting and filtering by quality signals`, `P8-004 Add shortlist, notes, and statuses`.

After Phase 7 closeout, Phase 7.5 was inserted as the QA gate before Phase 8. Phase 7.5 is now closed with the readiness decision `ready after approved fixes completed`. Phase 8 is active; `P8-001` is completed as the Candidate Workspace v0 contract and `P8-002 Build recruiter-facing candidate table`, `P8-003 Add sorting and filtering by quality signals`, and `P8-004 Add shortlist, notes, and statuses` are implemented. The current workspace is frontend-only: `P8-002` created the mapper, workspace run state, recruiter-facing table/list, and read-only candidate details; `P8-003` added workspace view sorting/filtering over `workspaceCandidates`; `P8-004` added frontend review state, derived shortlist, escaped plain-text notes, and review-status/shortlist view filters. Explanations, export, persistence, and backend/API/search/runtime changes remain out of scope. Historical Phase 7.5 QA used Tavily only when a scenario required it, and only through the existing approved backend pipeline and explicit `Approve & Search` flow: recruiter chat -> Search Brief -> Agent Plan -> Build Plan -> visible QueryPlan -> explicit approval -> approved Tavily-backed results.
...
Phase 4 is completed as AI Agent Foundation. Phase 5 is completed as the narrow Java/Ukraine Agent UX foundation. Phase 5.5 is completed as technical preparation before runtime. Phase 6 is completed as `AI Agent Runtime v0 baseline`. Phase 7 is completed as `Agent Conversation Wording Layer v0 baseline`. Phase 7.5 is completed as a recruiter simulation QA gate with the decision `ready after approved fixes completed`. Phase 8 is now active, and `P8-001 Define candidate workspace contract`, `P8-002 Build recruiter-facing candidate table`, `P8-003 Add sorting and filtering by quality signals`, and `P8-004 Add shortlist, notes, and statuses` are completed as the first frontend-only Candidate Workspace slice. Every following task should intentionally move the product toward a real AI Agent experience while preserving backend tool boundaries and explicit approval before execution.

...
Current phase: Phase 9.15 `Pending Search Brief Update Backend Routing` is completed through `P9.15-001`; Phase 10+ is parked unless the POC is explicitly reopened.
...
Completed Phase 8.5 tasks: `P8.5-001 Define agentic candidate review contract`, `P8.5-002 Add top-candidate recommendation from returned workspace facts`, `P8.5-003 Add selected-candidate comparison`, `P8.5-004 Add fit/gap explanation across selected candidates`, `P8.5-005 Add guided next-refinement suggestions from workspace results`.

Completed Phase 8.75 tasks: `P8.75-001 Run recruiter UAT acceptance gate before persistence`, `P8.75.1-001 Run UI conversation UX UAT before persistence`.

Phase 8.8 is completed as a recruiter-facing conversation hardening slice before persistence. Completed items are `P8.8-001` through `P8.8-024`. The implementation adds bounded intent classification, an evidence gate against hallucinated Search Brief readiness, safe conversational wording fallback/overlay, chat-confirmed pending-action intent handling, hidden technical preparation panels, Java-programmer support classification, Search Brief field explanation intent, pending-field answer classification, safer pending-action refinement handling, pending-hypothesis confirmation, pending-update field selection, bounded restart/start-over handling, immediate user-message echo with assistant thinking state, suppression of redundant post-update confirmation bubbles, direct explicit update/refine routing, and a simpler candidate workspace with hidden primary sort/filter/export/review/diagnostic controls, fixed-height pagination, and a compact-chat/results-dominant layout. Deterministic hard safety guardrails and backend/runtime validation remain the authority layer.

Phase 8.9 is completed as recruiter-facing UI polish before persistence. Completed items are `P8.9-001` through `P8.9-003`. The implementation removes stale `Thinking...` chat bubbles once a real assistant/progress reply renders, changes the primary `Candidate Results` surface to a compact table with `Score`, `Name`, `Role`, `Location`, `Stack`, `Source`, and `Status`, defaults visible candidate ordering to score descending, keeps pagination/bounded results, and aligns the desktop results panel height with the recruiter chat panel.

Phase 9 is completed as the first multi-provider search expansion. Completed items are `P9-001` through `P9-008`. The approved backend runtime/search execution path now expands beyond Tavily by adding Serper, SerpApi Google with a 5-page limit, and SerpApi Bing with a 5-page limit. Provider outputs normalize into the existing candidate pipeline, cross-provider dedupe remains based on normalized public LinkedIn profile URL, deduped candidates retain provider provenance, and reports include provider breakdown/counts/limits. Provider failures are bounded and no provider keys are logged. The implementation preserves the absolute boundaries: no frontend direct provider calls, no direct web-search agent bypass, no LinkedIn login/access automation/scraping, no candidate messaging, no account actions, no autonomous execution, and no persistence/database changes.

Phase 9.5 is completed as the final POC hardening/deploy phase. `P9.5-001` through `P9.5-007` are implemented and verified: Phase 10+ is parked, recruiter input is English-only, search is generalized beyond the Java/Ukraine baseline to any English IT role with role/main technology/stack/location, Candidate Results has polished empty/loading states, duplicate ready/search wording and visible `Workspace Ready` are removed, final regression coverage is wired into `scripts/check_all.ps1`, and the POC is deployed to Render at `https://engineers-search-engine-poc.onrender.com/`. Search providers remain as currently implemented in Phase 9 unless a later reviewed task explicitly reopens that topic.
...
Phase 9.11 is completed as `Requirement-To-Search-Signal Resolver`. `P9.11-001` adds bounded backend resolution for non-developer IT recruiter briefs: explicit domain requirements such as banking can become the executable search anchor in `technology`, explicit work-skill/tool/language requirements such as English and Excel can become `stack`, and the original requirements remain preserved in `must_have`. Developer/engineer/QA-style roles remain blocked from this resolver. Chat still only prepares a Search Brief and never builds a QueryPlan or executes search automatically.

Phase 9.12 is completed as `Role Understanding Resolver`. `P9.12-001` adds bounded role-scope classification only for IT-adjacent, generic, invalid, or non-IT clean-state recruiter roles before backend Search Brief validation. The LLM classifies role scope and evidence only; backend validation remains authoritative.

Phase 9.13 is completed as `Help/SmallTalk Intent Resolver`. `P9.13-001` routes harmless help/opening messages through bounded intent classification before unclear-search fallback while preserving any current Search Brief and execution approval boundaries.

Phase 9.14 is completed as `Search Brief Update Replacement Resolver`. `P9.14-001` generalizes natural-language value replacements for existing Search Briefs: bounded `PendingAnswerInterpreter v2` extracts `replace_value`, `old_value`, optional `field`, and replacement value; backend resolution matches the old value against the current Search Brief, validates the new value by resolved field, asks narrow clarification on missing/ambiguous old values, and applies only existing `brief_patch` operations without building QueryPlans or executing search.

Phase 9.15 is completed as `Pending Search Brief Update Backend Routing`. `P9.15-001` corrects frontend pending-update ownership: restart/cancel and short field selections such as `stack` remain local, but broad pending update text such as `change SQL to React` clears the local pending action and routes through `/api/recruiter-chat/turn`, allowing the backend semantic update resolver from P9.14 to validate and patch the current Search Brief. Selected-field follow-up values still pass `pending_update_field`; no QueryPlan or search starts automatically.
...
`P8-032` is closed as the parent recruiter-facing AI conversation and workspace presentation umbrella. Its observed child issues are either directly implemented or demonstrably covered by the reviewed slices and bundles: `P8-017` through `P8-021`, `P8-023` through `P8-031`, and separate `P8-022` multi-wave default behavior. Phase 8.5, Phase 8.75, and Phase 8.8 are now also completed; the next reviewed direction is Phase 9 multi-provider search expansion, without autonomous execution, direct LinkedIn access, scraping, messaging, account actions, or frontend/direct web-search bypass.
...
`P8.5-004` is implemented as deterministic frontend-only fit/gap explanation over current visible shortlisted workspace candidates. It adds `candidateWorkspace.buildSelectedCandidateFitGapExplanation()`, renders a compact fit/gap block, and is covered by `scripts/smoke_p85_selected_candidate_fit_gap.py`. The helper uses deterministic candidate explanation reasons plus bounded deterministic markers so capped reason lists do not hide visible role/technology fit. Missing evidence is phrased as not visible / needs manual review, foreign or mismatched location remains a caution, and the output returns no URL-derived `candidate_id`, profile URLs/normalized URLs, raw snippets/content, recruiter notes, or account identifiers. It does not add backend/LLM/Tavily/LinkedIn/profile-opening/storage behavior and does not change candidate facts, scoring, filters, dedupe, export, Search Brief, QueryPlan, runtime approval, or search execution.

`P8.5-005` is implemented as deterministic frontend-only review/refinement guidance over current visible workspace candidates. It adds `candidateWorkspace.buildWorkspaceRefinementSuggestions()`, renders a compact review guidance block, and is covered by `scripts/smoke_p85_workspace_refinement_suggestions.py`. Suggestions are derived from visible candidate stats and review-status counts only, contain no `brief_patch`, `proposed_action`, endpoint, action button, approval flag, or executable next step, and tell the recruiter to write changes in chat if search criteria need to change. The output returns no URL-derived `candidate_id`, profile URLs/normalized URLs, raw snippets/content, recruiter notes, or account identifiers. It does not add backend/LLM/Tavily/LinkedIn/profile-opening/storage behavior and does not change candidate facts, scoring, filters, dedupe, export, Search Brief, QueryPlan, runtime approval, or search execution.

`P8.75-001` is completed as the Recruiter UAT Acceptance Gate before Phase 9 multi-provider expansion and Phase 10 persistence. The no-live runner covers deterministic recruiter chat, Search Brief, Agent Plan, QueryPlan, runtime guardrail, static contract, and Candidate Workspace/Phase 8.5 helper acceptance checks and is wired into `scripts/check_all.ps1`; final no-live UAT passed `326/326` checks. The limited live runner uses only the existing backend Agent Runtime prepare -> explicit approval -> execute_approved path and records aggregate counts only; final live UAT passed `30/30` checks across 2 approved backend-runtime executions.

`P8.75.1-001` is completed as the UI Conversation UX UAT before Phase 9 multi-provider expansion and Phase 10 persistence. The runner drives the real frontend with Playwright, types simulated recruiter messages into `Recruiter Chat`, uses deterministic OpenAI/Tavily doubles, and passed `116/116` scenarios across positive, incomplete, noisy, off-topic, prohibited, refinement, confirmation, and post-search flows. The gate fixed harsh harmless-input wording, Russian public wording leaks, stale executable state after prohibited requests, natural EN/RU confirmation detection, and visible progress-copy leaks without adding persistence, new providers, autonomous execution, direct LinkedIn access/login/scraping, candidate messaging, account actions, or live external calls.

Bundle A (`P8-031 Make candidate table the primary post-search surface` + `P8-030 Rebalance desktop layout toward candidate workspace`) is implemented, then superseded in primary-row presentation by Phase 8.8 follow-up cleanup. Candidate Results render before Search summary, the desktop shell gives the right-side candidate workspace more width, and candidate rows remain focused on score/identity/role/location/stack/source. After `P8.8-019` through `P8.8-024`, visible sort/filter/export controls, agentic guidance blocks, row-level status/shortlist controls, diagnostic badges, and expandable diagnostic details are hidden from the primary recruiter view while underlying workspace state/helpers remain available for later reviewed surfaces.
...
- use Phase 9.5 as the reviewed place to expand the POC search scope beyond the narrow Java/Ukraine baseline;
...
- Phase 8.75 is completed as the recruiter UAT acceptance gate before Phase 9 multi-provider expansion and Phase 10 persistence; `P8.5-001 Define agentic candidate review contract`, `P8.5-002 Add top-candidate recommendation from returned workspace facts`, `P8.5-003 Add selected-candidate comparison`, `P8.5-004 Add fit/gap explanation across selected candidates`, `P8.5-005 Add guided next-refinement suggestions from workspace results`, `P8.75-001 Run recruiter UAT acceptance gate before persistence`, and `P8.75.1-001 Run UI conversation UX UAT before persistence` are completed;
...
- keep candidate workspace/shortlist for Phase 8 and database/persistent memory for Phase 10.
...
- Phase 8.5: `Agentic Candidate Review & Iteration`, completed through top-candidate analysis, selected-candidate comparison, fit/gap explanation, and guided refinement from already returned workspace facts only.
...
- Phase 9.14: `Search Brief Update Replacement Resolver`, completed through `P9.14-001` so natural replacement requests such as `Change Angular skill to PHP` and `update Selenium to Cucumber` are handled by bounded semantic extraction plus backend old-value matching/validation before existing `brief_patch` operations.
- Phase 9.15: `Pending Search Brief Update Backend Routing`, completed through `P9.15-001` so pending update free text routes to backend semantic update resolution while local frontend handling remains limited to restart/cancel and clear short field selection.
...
- Phase 9.9: `AI Agent Semantic Understanding Hardening`; completed through `P9.9-007`. Current correction path: bounded LLM clean-state Search Brief extraction/refinement interpretation with backend validation, domain/technology separation, automated semantic recruiter UAT, and closeout decision.
...
- Clean-state initial Search Brief extraction and existing-draft Search Brief refinement now use bounded LLM interpretation plus strict backend validation, including separation between domain/business context and technical skills. Automated semantic recruiter UAT and refinement smoke coverage are in the local regression baseline.

...
`P8-008` through `P8-015` are implemented as bounded current-flow chat-quality hardening. The batch adds optional bounded LLM onboarding wording with deterministic fallback, deterministic pre-extraction off-topic/noise guardrails, the approved conservative classification policy in code, Russian pending-stack clarification answers, RU/EN next-iteration option localization, frontend/session-only chat-confirmed `Build Plan`, Enter-to-send, and unified visible `AI Assistant` speaker titles. This batch preserves the explicit `Approve & Search` Tavily boundary and does not add autonomous execution, direct web-search bypass, direct LinkedIn access/login/scraping, candidate messaging, account actions, persistence, memory, new sources, or country/technology expansion. `P8-016 Harden pending clarification answer routing` is added to backlog as a draft task for the observed `сантехника`-while-waiting-for-location issue; it is not approved or implemented.

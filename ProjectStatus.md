...
Phase 10+ persistence and later feature tracks are parked for now. The current decision is to close the project as a POC after a final hardening/deploy slice. The working plan is recorded in `docs/phase-9-5-final-poc-plan.md`.
...
Phase 9.5 is the active final POC hardening/deploy phase. Planned items are `P9.5-001` through `P9.5-007`: park Phase 10+ and define final POC closeout, enforce English-only recruiter input, generalize search beyond the Java/Ukraine baseline to any English IT role with role/main technology/stack/location, polish empty/loading Candidate Results states, remove duplicate ready/search wording and visible `Workspace Ready`, run final POC regression, and deploy to Render. Search providers remain as currently implemented in Phase 9 unless a later reviewed task explicitly reopens that topic.
...
- Phase 9.5: `Final POC Hardening And Render Deployment`, current active final POC phase; Phase 10+ is parked.

...
Current phase: `Phase 9.5 - Final POC Hardening And Render Deployment`.

Phase 10+ persistence and later feature tracks are parked for now. The current decision is to close the project as a POC after a final hardening/deploy slice. The working plan is recorded in `docs/phase-9-5-final-poc-plan.md`. Phase 9.5 code hardening is implemented and locally verified; live Render service creation is pending Render credentials/dashboard setup.
...
Phase 9.5 is the final POC hardening/deploy phase. `P9.5-001` through `P9.5-006` are implemented and locally verified: Phase 10+ is parked, recruiter input is English-only, search is generalized beyond the Java/Ukraine baseline to any English IT role with role/main technology/stack/location, Candidate Results has polished empty/loading states, duplicate ready/search wording and visible `Workspace Ready` are removed, and final regression coverage is wired into `scripts/check_all.ps1`. `P9.5-007` has the deployment path prepared with `render.yaml` and `docs/render-deployment.md`; live Render service creation remains pending Render credentials/dashboard setup. Search providers remain as currently implemented in Phase 9 unless a later reviewed task explicitly reopens that topic.
...
- Phase 9.5: `Final POC Hardening And Render Deployment`, code hardening implemented and locally verified; Phase 10+ is parked, and live Render service creation remains pending Render credentials/dashboard setup.

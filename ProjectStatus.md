...
Current phase: Phase 9.7 `Recruiter Chat Semantic Interpreter` is planned as the next architectural correction; Phase 9.6 is completed through `P9.6-008`; Phase 10+ is parked.
...
Phase 9.7 is planned as `Recruiter Chat Semantic Interpreter`. It should introduce a bounded LLM semantic interpreter contract, strict backend validator, incremental stack/location/update-refinement rollout, legacy pending-handler cleanup, and recruiter semantic conversation UAT. The target pipeline is `message -> deterministic safety precheck -> bounded LLM semantic interpreter -> strict backend validator -> deterministic patch/action`.

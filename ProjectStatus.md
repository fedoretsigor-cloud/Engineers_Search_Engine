...
Current phase: Phase 9.6 post-deploy recruiter UX polish is completed; Phase 10+ is parked.
...
Phase 9.6 is completed as post-deploy recruiter UX polish. `P9.6-001` through `P9.6-006` show a manually clickable safe LinkedIn profile link under each candidate name/headline, remove low-value placeholder-heavy `Location`/`Stack` columns from the primary table, make the initial chat helper higher/shorter/italic, make the message input taller, make chat action buttons shorter, and allow non-hardcoded English IT/software-relevant stack signals through bounded LLM validation when deterministic recognition is insufficient. The slice preserves no LinkedIn automation, no scraping, no messaging, no account actions, no backend search execution boundary changes, and no persistence.
...
Phase 9.6 is completed as post-deploy recruiter UX polish. `P9.6-001` through `P9.6-006` are implemented and verified: Candidate Results shows manual safe LinkedIn profile links under candidate identity, the primary table no longer shows `Location` and `Stack` placeholder columns, the initial chat helper is compact/high/italic, the chat textarea is taller, chat action buttons are shorter, and pending stack clarification accepts bounded non-hardcoded English IT/software stack signals via LLM classification with deterministic safe fallback. `scripts/check_all.ps1`, the dedicated `scripts/smoke_p96_post_deploy_polish.py`, and local Playwright browser sanity passed.
...
- Phase 9.6: `Post-Deploy Recruiter UX Polish`, completed through `P9.6-006`.
